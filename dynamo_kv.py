import bisect, dataclasses, hashlib, time, typing
        ...
        for nid in preferred:
            if self.node(nid).is_alive():
                ...
                if len(responses) >= self.R:
                    break
        all_versions = []
        [all_versions.extend(vs) for _, vs in responses]
...
def _demo():
    ...
    # resolve
    items = []
    [items.extend(v['items']) for v in r2['values']]
    ...
    print(
        'Resolved PUT:',
        cluster.put(key2, {'items': items}, context=vc, coordinator_id='C'),
    )
